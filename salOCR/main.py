## Implementation for bulk processing of Hindi PDF files to text
## Create a folder with PDF files and store it in the salOCR directory
## Run this file : python main.py
## After processing, the output combined text file will be stored inside a new folder within the salOCR directory
## Go through the 'processing.txt' file to understand what to expect during the processing
...
def process_pdf(pdf_path, total_pdfs, pdf_index):
    """Process a single PDF and extract text from all pages."""
    print(f"Processing PDF {pdf_index + 1} of {total_pdfs}: {os.path.basename(pdf_path)}")
    extracted_text = ""
    try:
        pages = pdf2image.convert_from_path(pdf_path, dpi=300, grayscale=True)
        total_pages = len(pages)
        for page_number, page in enumerate(tqdm(pages, desc=f"Processing Pages for {os.path.basename(pdf_path)}"), start=1):
...
def main():
    pdf_folder = "your_pdf_folder"  # Folder containing PDF files
    ...
    for pdf_index, pdf_file in enumerate(pdf_files):
        ...
        pdf_text = process_pdf(pdf_path, total_pdfs, pdf_index)
        all_text += f"\n--- {pdf_file} ---\n{pdf_text}\n"
